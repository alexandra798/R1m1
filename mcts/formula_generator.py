...
def generate_formula(all_features):
    ...
    if formula_type == 'binary':
        ...
        ts_op = np.random.choice(['ts_ref', 'ts_rank', 'ts_std', 'ts_max', 'ts_min', 'ts_skew',
                                  'ts_kurt', 'ts_mean', 'ts_med', 'ts_sum', 'decay_linear', 'ts_wma', 'ts_ema'])
        
        if ts_op == 'ts_ref':
            formula = f"ts_ref({feature}, {t})"
        else:
        ...
        if mix_type == 'ts_binary':
            ...
            feature2 = np.random.choice(all_features)
            t = np.random.choice(time_deltas)
            ts_op = np.random.choice(['ts_mean', 'ts_std', 'ts_max', 'ts_min'])
            operator = np.random.choice(binary_operators)
            ...
            formula = f"greater({feature1}, ts_mean({feature1}, {t})) * {feature2}"
