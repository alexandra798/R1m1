    ...
    def apply_time_series_op(op_name, data, window):
        ...
        if isinstance(data, pd.Series):
            if op_name == 'ts_ref':
                ...
                # Std(x,t): 过去t天的标准差
                return data.rolling(window=window, min_periods=2).std().fillna(0)

            elif op_name == 'ts_var':
                # Var(x,t): 过去t天的方差
                return data.rolling(window=window, min_periods=2).var().fillna(0)
                ...
                # Skew(x,t): 过去t天的偏度
                return data.rolling(window=window, min_periods=3).skew().fillna(0)

            elif op_name == 'ts_kurt':
                # Kurt(x,t): 过去t天的峰度
                return data.rolling(window=window, min_periods=4).kurt().fillna(0)
            ...
            for i in range(len(data)):
                ...
                if op_name == 'ts_ref':
                    ...
                    result[i] = np.var(window_data) if len(window_data) > 1 else 0
